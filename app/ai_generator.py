  ...
  "questions": [
    {
      ...
      "correct": "A",
      "explanation": "מדוע נכונה: [הסבר מפורט]\\nמדוע B שגויה: [הסבר]\\nמדוע C שגויה: [הסבר]\\nמדוע D שגויה: [הסבר]\\nמקור: [עמוד/פרק ספציפי]",
      "difficulty": "medium",
      "clinical_task": "apply"
    }
  ]
...
חומר מקצועי רלוונטי מתוך ספרי הלימוד:
---
{content}
---

הנחיות חשובות:
- צור שאלות רק על תת-הנושאים שנבחרו למעלה. אל תסטה לנושאים אחרים שלא מופיעים ברשימה.
- בסס את השאלות על התכנים שלעיל, אך אל תתמקד בהגדרות ובקריטריונים אבחנתיים. השתמש בהם כרקע לתיאור המקרה בלבד.
- כל שאלה חייבת לעסוק בתחום מוקד אחר - אסור ששתי שאלות יעסקו באותו תחום (למשל: אם שאלה אחת על טיפול תרופתי, הבאה חייבת להיות על נושא אחר)
- שלב תסריטים קליניים מציאותיים עם גילאים מגוונים (פעוטות, ילדי גן, ילדי בית ספר, מתבגרים) ומינים שונים
- העדף מצבים קליניים שכיחים ורלוונטיים לפרקטיקה היומיומית; שלב גם מצבים נדירים אך קריטיים לזיהוי
- כל שאלה צריכה להיות עצמאית ושונה מהאחרות - הימנע מחזרה על אותו תבנית או תרחיש
- ציין בהסבר הפניה לחומר המקור (ספר/פרק) כשאפשר
...
def generate_questions(topic_id, count=3, difficulty='medium', clinical_task='mixed', subtopic_ids=None):
    ...
    if not content:
        content = 'לא נמצא חומר ספציפי - צור שאלות על בסיס הידע הכללי שלך בנושא.'
